...
arm.set_servo_angle(angle=init_qpos, speed=0.5, is_radian=True)  # 初始化速度 0.5 rad/s，避免过快
...
try:
    with mujoco.viewer.launch_passive(mjmodel, mjdata) as viewer:
        ...
        while viewer.is_running() and time.time() - start < 1000:
            ...
            t += 1
            target_pose, gripper_state_r, _, roll_change, home_button = joyconrobotics_right.get_control()
            ...
            if IK_success:
                ...
                if home_button == 1:
                    if roll_diff > -0.04 and roll_diff < 0.04:
                        roll_diff = 0
                    elif roll_diff < 0:
                        roll_diff += 2 * ROLL_SPEED
                    elif roll_diff > 0:
                        roll_diff -= 2 * ROLL_SPEED
                ...
                if gripper_state_r == 1:
                    curr_action_msg = np.append(np.rad2deg(command_angles_rad), 840)
                else:
                ...
                arm.set_servo_angle(angle=command_angles_rad,speed=40,is_radian=True)

                if gripper_state_r == 1:
                    # print("gripper open")
                    arm.set_gripper_position(pos=840, wait=False)
                else:
                    # print("gripper close")
