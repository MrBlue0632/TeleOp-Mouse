...
class JoyconRobotics:
    ...
    def common_update(self):
        ...
        joycon_button_y_inc = self.joycon.get_button_y() if self.joycon.is_right() else self.joycon.get_button_left()
        joycon_button_y_dec = self.joycon.get_button_a() if self.joycon.is_right() else self.joycon.get_button_right()
        if joycon_button_y_inc == 1:
            self.position[1] += 0.001 * self.dof_speed[1]
        elif joycon_button_y_dec == 1:
            self.position[1] -= 0.001 * self.dof_speed[1]

        self.roll_change = 0  # Y/A 已改为控制 Y 轴，roll 不再由此控制
    ...
    def get_control(self, out_format="euler_rad"):
        ...
        return self.posture, self.gripper_state, self.button_control, self.roll_change, self.home_button
